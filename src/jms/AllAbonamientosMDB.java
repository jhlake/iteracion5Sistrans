...
public class AllAbonamientosMDB implements MessageListener, ExceptionListener
{
	...
	private final static String LOCAL_TOPIC_NAME = "java:global/RMQAbonamientosLocal";
	...
	private List<Abonamiento> answer = new ArrayList<Abonamiento>();
	...
	public ListaAbonamientos getRemoteAbonamientos() throws JsonGenerationException, JsonMappingException, JMSException, IOException, NonReplyException, InterruptedException, NoSuchAlgorithmException
	{
		answer.clear();
		...
		ListaAbonamientos abo = new ListaAbonamientos(answer);
        return abo;
	}
		...
		try 
		{
			...
			if(!ex.getSender().equals(APP))
			{
				if(ex.getStatus().equals(REQUEST))
				{
					...
					ListaAbonamientos abo = dtm.getLocalAbonamientos();
					String payload = mapper.writeValueAsString(abo);
					Topic t = new RMQDestination("", "festival.test", ex.getRoutingKey(), "", false);
					sendMessage(payload, REQUEST_ANSWER, t, id);
				}
				else if(ex.getStatus().equals(REQUEST_ANSWER))
				{
					ListaAbonamientos v = mapper.readValue(ex.getPayload(), ListaAbonamientos.class);
					answer.addAll(v.getAbonamientos());
				}
			}
