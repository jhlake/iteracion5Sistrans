...
public class AllRetirarCompaniaMDB implements MessageListener, ExceptionListener 

	{
		public final static int TIME_OUT = 5;
		private final static String APP = "app1";
		
		private final static String GLOBAL_TOPIC_NAME = "java:global/RMQTopicAllRetirarCompaniaMDB";
		private final static String LOCAL_TOPIC_NAME = "java:global/RMQAllRetirarCompaniaLocal";
		...
		private List<CompaniaDeTeatro> answer = new ArrayList<CompaniaDeTeatro>();
		
		public AllRetirarCompaniaMDB(TopicConnectionFactory factory, InitialContext ctx) throws JMSException, NamingException 
		{	
		...
		public ListaCompanias deleteRemoteCompanias() throws JsonGenerationException, JsonMappingException, JMSException, IOException, NonReplyException, InterruptedException, NoSuchAlgorithmException
		{
			answer.clear();
			...
			ListaCompanias res = new ListaCompanias(answer);
	        return res;
		}
			...
			try 
			{
				...
				if(!ex.getSender().equals(APP))
				{
					if(ex.getStatus().equals(REQUEST))
					{
						...
						ListaCompanias videos = dtm.deleteLocalCompanias();
						...
						{
							ListaCompanias v = mapper.readValue(ex.getPayload(), ListaCompanias.class);
							answer.addAll(v.getCompanias());
						}
					}
